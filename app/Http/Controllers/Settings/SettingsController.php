...
class SettingsController extends Controller
{
    ...
    {
        $this->middleware(['auth','web','permission:Операции-с-настройками-сайта','2fa']);
    }
    /**
    ...
    public function appNameUpdate(Request $request)
    {
      $this->validate($request,[
        'app_name' => 'required|min:4|regex:/^[A-Za-zа-яёА-ЯЁ0-9_.,()№\/\\-:;\'"— ]+$/u',
      ],[
        'app_name.regex' => 'Название сайта может содержать только буквы, цифры, пробелы и специальные знаки (_ ( ) . , № \ / : ; \' " —).',
        'app_name.min' => 'Минимальная длина названия сайта - 4 символа',
          'app_name.required' => 'Название сайта - необходимое поле'
      ]);
        ...
        return redirect()->back()->with('success', 'Название сайта успешно изменено');
    }
    ...
    public function appLogoUpdate(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'Логотип сайта успешно изменён');
    }
    ...
    public function appThemeUpdate(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'Тема сайта успешно изменена');
    }
    ...
    public function authSettingsUpdate(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'Настройки аутентификации успешно обновлены');
    }
    ...
    public function backupFiles()
    {
        ...
        if (Str::contains($output, 'Backup completed!')) {
            return redirect()->back()->with('success', 'Резервная копия файлов сайта успешно выполнена');
        } else {
            return redirect()->back()->with('error', 'Резервное копирование файлов сайта не удалось');
        }
    }
    ...
    public function backupDb()
    {
        ...
        if (Str::contains($output, 'Backup completed!')) {
            return redirect()->back()->with('success', 'Резервное копирование базы данных сайта успешно завершено');
        } else {
            return redirect()->back()->with('error', 'Резервное копирование базы данных сайта не удалось');
        }
    }
    ...
    public function deleteBackup($name, $ext)
    {
        ...
        if ($status) {
            return redirect()->back()->with('success', 'Резервная копия успешно удалена');
        } else {
            return redirect()->back()->with('error', 'Упс! Произошла ошибка, попробуйте снова');
        }
    }
