<nav class="main-header navbar navbar-expand navbar-{{setting('app_theme')}}">
    ...
    <ul class="navbar-nav ml-auto">
        @impersonating
        <li class="nav-item">
            <a class="nav-link text-danger btn btn-none" href="{{ route('impersonate.leave') }}">
        ...
        <li class="nav-item dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                ...
                @can('Операции-с-настройками-сайта')
                ...
                <a href="/logout" class="dropdown-item dropdown-footer bg-gray"><i
                        class="fa fa-sign-out mr-2"></i> {{ __('app.logout') }}</a>
            </div>
        </li>
    </ul>
</nav>
...
<aside class="main-sidebar sidebar-{{setting('app_sidebar')}}-light elevation-4">
    ...
    <div class="sidebar">
        ...
        <div class="user-panel mt-3 pb-3 mb-3 d-flex">
            <div class="image">
                <a href="/profile"><img
                        src="{{Auth::user()->avatar?Auth::user()->avatar:asset('uploads/avatar/avatar.png')}}"
        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                ...
                <!--
                @can('Доступ-к-панели-управления')
                <li class="nav-item">
                ...
                @can('Просмотр-пользователей')
                    <li class="nav-item">
                        <a href="{{route('user.index')}}"
                           class="nav-link {{request()->is('user*')? 'sidebar-active':''}}">
                ...
                @can('Операции-с-группами-пользователей')
                    <li class="nav-item">
                        <a href="/groups"
                           class="nav-link {{request()->is('groups*')? 'sidebar-active':''}}">
                ...
                @can('Операции-с-ролями-и-разрешениями')
                    <li class="nav-item">
                        <a href="{{route('role.index')}}"
                           class="nav-link {{request()->is('role*')? 'sidebar-active':''}}">
                            <i class="fa fa-key nav-icon"></i>
                            <p>{{ __('app.roles_and_permissions') }}</p>
                        </a>
                    </li>
                @endcan
                ...
                <!--li class="nav-item has-treeview">
                    <a href="#"
                       class="nav-link {{(request()->is('subscription/plan')||request()->is('/subscription-income')||request()->is('/subscribed-users'))? 'sidebar-active':''}}">
                    ...
                    <ul class="nav nav-treeview">
                        <li class="nav-item">
                            <a href="/subscription/plan"
                               class="nav-link {{request()->is('subscription/plan')? 'sidebar-active':''}}">
                        ...
                        <li class="nav-item">
                            <a href="/subscription-income"
                               class="nav-link {{request()->is('subscription-income')? 'sidebar-active':''}}">
                                <i class="fa fa-circle ml-3 mr-1"></i>
                        ...
                        <li class="nav-item">
                            <a href="/subscribed-users"
                               class="nav-link {{request()->is('subscribed-users')? 'sidebar-active':''}}">
                                <i class="fa fa-circle ml-3 mr-1"></i>
                        ...
                        <li class="nav-item">
                            <a href="/checkout-sample"
                               class="nav-link {{request()->is('checkout-sample*')? 'sidebar-active':''}}">
                                <i class="fa fa-circle ml-3 mr-1"></i>
                ...
                @if(setting('stripe_status'))
                    <!--li class="nav-item">
                        <a href="{{route('/subscription')}}"
                           class="nav-link {{request()->is('subscription*')? 'sidebar-active':''}}">
                ...
                <li class="nav-item">
                    <a href="{{route('settings.index')}}"
                       class="nav-link {{request()->is('settings')? 'sidebar-active':''}}">
                ...
                <!--li class="nav-item has-treeview">
                    <a href="/category-article"
                       class="nav-link {{request()->is('category-article*')? 'sidebar-active':''}}">
                        <i class="nav-icon fa fa-newspaper-o"></i>
                        <p>
                ...
                <!--li class="nav-item">
                    <a href="/premium-content"
                       class="nav-link {{request()->is('premium-content*')? 'sidebar-active':''}}">
                ...
                <li class="nav-item has-treeview">
                    <a href="/cases" class="nav-link {{(request()->is('cases*') && !request()->is('cases/archive*')) ? 'sidebar-active':''}}">
                        <i class="nav-icon fa fa-briefcase"></i>
                        <p>
                            {{ __('app.business_cases') }}
                        </p>
                    </a>
                </li>

                @can('Операции-с-папками')
                    <li class="nav-item has-treeview">
                        <a href="/folders" class="nav-link {{request()->is('folders*') && !request()->is('folders/archive*') ? 'sidebar-active':''}}">
                ...
                @can('Операции-с-папками')
                    <li class="nav-item has-treeview">
                        <a href="/folder-tree" class="nav-link {{request()->is('folder-tree*') ? 'sidebar-active':''}}">
                ...
                @can('Операции-с-архивом')
                    <li class="nav-item has-treeview">
                        <a href="/cases/archive" class="nav-link {{request()->is('cases/archive*')? 'sidebar-active':''}}">
                            <i class="nav-icon fa fa-archive"></i>
                            <p>
                                {{ __('app.business_cases_archive') }}
                            </p>
                        </a>
                    </li>
                @endcan
                ...
                @can('Операции-с-архивом')
                    <li class="nav-item has-treeview">
                        <a href="/folders/archive" class="nav-link {{request()->is('folders/archive*') ? 'sidebar-active':''}}">
                            <i class="nav-icon fa fa-trash"></i>
                            <p>
                                {{ __('app.folders_archive') }}
                            </p>
                        </a>
                    </li>
                @endcan

                @can('Доступ-к-базе-связей')
                <li class="nav-item has-treeview">
                    <a href="/connections" class="nav-link {{request()->is('connections*')? 'sidebar-active':''}}">
                        <i class="nav-icon fa fa-users"></i>
                        <p>
                            {{ __('app.connections_database') }}
                        </p>
                    </a>
                </li>
                @endcan
